			...
			while(--size >= 0) {
				...
				if(p.l  - 1 >= 0) {
					...
					for(int i = 1; i <= 3; i++) {
						...
						if(range(p.x, p.y, nd) && !chk[p.x + dx[nd]][p.y + dy[nd]][p.l - i][p.r]) {
							chk[p.x + dx[nd]][p.y + dy[nd]][p.l - i][p.r] = true;
							q.offer(new Pair(p.x + dx[nd], p.y + dy[nd], p.l - i, p.r));
						}
					}
				}
				if(p.r - 1 >= 0) {
					...
					for(int i = 1; i <= 3; i++) {
						...
						if(range(p.x, p.y, nd) && !chk[p.x + dx[nd]][p.y + dy[nd]][p.l][p.r - i]) {
							chk[p.x + dx[nd]][p.y + dy[nd]][p.l][p.r - i] = true;
							q.offer(new Pair(p.x + dx[nd], p.y + dy[nd], p.l, p.r - i));
						}
					}
				}
			}
